...
function updateHeatmap() {
    ...
    d3.csv("covid.csv").then(function (data) {
      ...
      var myGroups = Array.from(new Set(data.map(d => d.age_group)));
      var myVars = Array.from(new Set(data.map(d => d.race_ethnicity_combined)));
      ...
      // Create tooltip
      var tooltip = d3.select("body") // Append to body or the container element
        .append("div")
        .style("opacity", 0)
        .attr("class", "tooltip")
      ...
      svg.selectAll()
        ...
        .attr("y", d => y(d.race_ethnicity_combined))
