...
function updateHeatmap() {
   var margin = { top: 30, right: 30, bottom: 50, left: 100 },
    ...
    d3.csv("covid.csv", function (data) {
        ...
        // Extract unique age groups (Y-axis) and race/ethnicity (X-axis)
        var ageGroups = Array.from(heatmapData.keys());
        var raceEthnicities = Array.from(
            new Set(data.map(d => d.race_ethnicity_combined))
        );
        ...
        // Build color scale
        var maxValue = d3.max(Array.from(heatmapData.values()), d =>
        ...
        var mousemove = function (event, d) {
            tooltip
                .html(
                    ...
                    "<br>Race/Ethnicity: " + d.race_ethnicity +
                    "<br>Count: " + d.value
                )
        ...
        svg.selectAll()
            .data(
                Array.from(heatmapData.entries()).flatMap(([age_group, races]) =>
                    Array.from(races.entries()).map(([race_ethnicity, value]) => ({
                        age_group,
                        race_ethnicity,
                        value
                    }))
