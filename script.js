...
function updatePieChart(attribute) {
    d3.csv("covid.csv").then(function(data) {
        ...
        var groupedData = d3.rollup(data, v => v.length, d => d[attribute]);
...
function updateHeatmap() {
    ...
    var x = d3.scaleBand().range([0, width]).domain(["A", "B", "C", "D", "E"]).padding(0.01);
    var y = d3.scaleBand().range([height, 0]).domain(["v1", "v2", "v3", "v4", "v5"]).padding(0.01);
